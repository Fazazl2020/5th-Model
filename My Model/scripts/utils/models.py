...
class Model(object):
    ...
    def test(self):
        ...
        print("\n" + "="*70)
        print("STEP 4: PROCESSING ALL SNR LEVELS")
        print("="*70 + "\n")
        ...
        for snr_idx, (snr_value, snr_noisy_dir) in enumerate(snr_dirs):
            print("\n" + "="*70)
            print(f"PROCESSING SNR {snr_value:+3d} dB ({snr_idx + 1}/{len(snr_dirs)})")
            print("="*70)
            
            logger.info('\n' + '='*70)
            logger.info(f'SNR {snr_value:+3d} dB - Starting Test')
            logger.info('='*70)
            ...
            logger.info(f'Clean directory: {TEST_CLEAN_DIR}')
            logger.info(f'Noisy directory: {snr_noisy_dir}')
            logger.info(f'Output directory: {snr_output_dir}')
            
            # Create test loader for this SNR
            try:
            ...
            with torch.no_grad():
                for k, batch in enumerate(test_loader):
                    try:
                        ...
                        # Get filename (remove .wav extension)
                        filename_base = os.path.splitext(batch['filenames'][0])[0]
                        
                        # Save to SNR-specific directory
                        sf.write(
                            os.path.join(snr_output_dir, f'{filename_base}_mix.wav'), 
                            mix_np, self.sample_rate
                        )
                        sf.write(
                            os.path.join(snr_output_dir, f'{filename_base}_sph.wav'), 
                            sph_np, self.sample_rate
                        )
                        sf.write(
                            os.path.join(snr_output_dir, f'{filename_base}_sph_est.wav'), 
                            sph_est_np, self.sample_rate
                        )
                        
                        if self.write_ideal:
                            sf.write(
                                os.path.join(snr_output_dir, f'{filename_base}_sph_idl.wav'), 
                        ...
                        # Progress logging
                        if (k + 1) % 50 == 0:
                            print(f'  Progress: {k + 1}/{len(test_loader)} samples ({100*(k+1)/len(test_loader):.1f}%)')
                            logger.info(f'  Progress: {k + 1}/{len(test_loader)} samples')
            ...
            if accu_n_frames > 0:
                ...
                logger.info(f'\n{"="*70}')
                logger.info(f'SNR {snr_value:+3d} dB - COMPLETED')
                logger.info(f'{"="*70}')
                ...
                # Count output files
                output_files = [f for f in os.listdir(snr_output_dir) if f.endswith('.wav')]
        ...
        print(f"  Total samples processed: {overall_stats['total_processed']}")
        print(f"  Total errors: {overall_stats['total_errors']}")
        
        print(f"\nResults organized in: {self.est_path}/")
        ...
        for snr_value in sorted(overall_stats['snr_results'].keys()):
            result = overall_stats['snr_results'][snr_value]
            logger.info(f'  SNR {snr_value:+3d} dB: {result["processed"]} samples, Loss: {result["avg_loss"]:.4f}')
        logger.info('='*70)
